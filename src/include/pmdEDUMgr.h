...
public:
  ...
  bool isQuiesced() { return _isQuiesced; }

  void setQuiesced(bool b) { _isQuiesced = b; }

  bool isDestroyed() { return _isDestroyed; }

  static bool isPoolable(EDU_TYPES type) { return (EDU_TYPE_AGENT == type); }
...
private:
  ...
  bool _isSystemEDU(EDUID eduID) {
    auto it = _mapSystemEDUS.find(eduID);
