...
int ossSocket::setSocketLi(int lOnOff, int linger) {
  ...
  rc = setsockopt(_fd, SOL_SOCKET, SO_LINGER, (const char *)&_linger,
                  sizeof(_linger));
  return rc;
}
...
int ossSocket::bind_listen() {
  ...
  if (rc) {
    PD_RC_CHECK(EDB_NETWORK, PDERROR, "Faile to listen socket, rc = %d", SOCKET_GETLASTERROR);
  }
...
int ossSocket::send(const char *pMsg, int len, int timeout, int flags) {
  ...
  while (true) {
    ...
    // if the socket we intreested is not receiving anything , let's continue
    if (FD_ISSET(_fd, &fds)) {
      break;
    }
  }
...
int ossSocket::recv(char *pMsg, int len, int timeout, int flags) {
  ...
  // wait look until either we timeout or get a message
  while (true) {
    FD_ZERO(&fds);
    FD_SET(_fd, &fds);
  ...
  while (len > 0) {
    // MSG_NOSIGNAL: Requests not to send SIGPIPE on errors on stream
    // oriented sockets when the other end breaks the connections. the EIPE
    // error is still returned
    ...
    if (rc > 0) {
      ...
      if ((EINTR == rc) && (retries < MAX_RECV_RETRIES)) {
        // less than max_recv_retries nuber, let's retry
        retries++;
        continue;
      }
...
int ossSocket::recvNF(char *pMsg, int &len, int timeout) {
  ...
  while (true) {
    ...
    // if the socket we intrerested is not receving anything, let's continue
    if (FD_ISSET(_fd, &fds)) {
      break;
    }
  }
  ...
  if (rc > 0) {
    ...
    PD_RC_CHECK(EDB_NETWORK_CLOSE, PDERROR, "Peer unexpected shutdown");
  } else {
    // if rc < 0
    rc = SOCKET_GETLASTERROR;
    if ((EAGAIN == rc || EWOULDBLOCK == rc) && _timeout > 0) {
...
int ossSocket::_getAddress(sockaddr_in *addr, char *pAddress,
                           unsigned int length) {
  int rc = EDB_OK;
...
unsigned int ossSocket::getLocalPort() { return _getPort(&_sockAddress); }

unsigned int ossSocket::getPeerPort() { return _getPort(&_peerAddress); }
...
int ossSocket::setTimeout(int seconds) {
  ...
  tv.tv_sec = seconds;
  tv.tv_usec = 0;
  // windows take millionsecnds as parameter
