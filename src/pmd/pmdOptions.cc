...
int pmdOptions::readCmd(int argc, char **argv, po::options_description &desc,
                        po::variables_map &vm) {
  int rc = EDB_OK;
  try {
...
int pmdOptions::readConfigureFile(const char *path,
  ...
  try {
    ...
    std::cerr << "Failed to open config file: " << (std::string)conf
              << std::endl;
    rc = EDB_IO;
    goto error;
  } catch (po::unknown_option &e) {
...
int pmdOptions::init(int argc, char **argv) {
  ...
  if (vm.count(PMD_OPTION_CONFPATH)) {
    rc = readConfigureFile(vm[PMD_OPTION_CONFPATH].as<std::string>().c_str(),
                           all, vm2);
  }
  ...
  rc = importVM(vm2);
  if (rc) {
    PD_LOG(PDERROR, "Failed to immport from vm2, rc = %d", rc);
    goto error;
  }

  // load vm form command line
  rc = importVM(vm);
  if (rc) {
    PD_LOG(PDERROR, "Failed to import from vm, rv = %d", rc);
    goto error;
  }
